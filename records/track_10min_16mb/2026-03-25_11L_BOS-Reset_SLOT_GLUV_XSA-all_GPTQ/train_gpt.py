...
class Hyperparameters:
    ...
    bos_reset_enabled = bool(int(os.environ.get("BOS_RESET_ENABLED", "1")))
    slot_ttt_enabled = bool(int(os.environ.get("SLOT_TTT_ENABLED", "1")))
    slot_ttt_lr = float(os.environ.get("SLOT_TTT_LR", "3e-4"))
    slot_ttt_steps = int(os.environ.get("SLOT_TTT_STEPS", "1"))
    checkpoint_dir = os.environ.get("CHECKPOINT_DIR", "")
    eval_from_checkpoint = os.environ.get("EVAL_FROM_CHECKPOINT", "")
def compute_cu_seqlens(input_ids: Tensor, bos_id: int = 1):
    """Compute cu_seqlens for flash_attn_varlen from BOS positions.
    Each BOS token starts a new document segment. Tokens before the first BOS
    in each batch item form their own segment. Attention is blocked across segments."""
    B, T = input_ids.shape
    is_bos = (input_ids == bos_id)
    seqlens = []
    for b in range(B):
        bos_pos = is_bos[b].nonzero(as_tuple=False).view(-1).tolist()
        seg_starts = [] if (bos_pos and bos_pos[0] == 0) else [0]
        seg_starts.extend(bos_pos)
        for i, start in enumerate(seg_starts):
            end = seg_starts[i + 1] if i + 1 < len(seg_starts) else T
            seqlens.append(end - start)
    seqlens_t = input_ids.new_tensor(seqlens, dtype=torch.int32)
    cu_seqlens = torch.zeros(len(seqlens) + 1, dtype=torch.int32, device=input_ids.device)
    cu_seqlens[1:] = seqlens_t.cumsum(0)
    max_seqlen = int(seqlens_t.max().item()) if seqlens_t.numel() > 0 else T
...
def eval_val(
    ...
    with torch.inference_mode():
        for batch_seq_start in range(seq_start, seq_end, local_batch_seqs):
            ...
            cu_seqlens, max_seqlen = (compute_cu_seqlens(x) if args.bos_reset_enabled else (None, None))
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                batch_loss = model(x, y, cu_seqlens, max_seqlen).detach()
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor | None = None, k_w: Tensor | None = None, v_w: Tensor | None = None, out_w: Tensor | None = None, v_embed: Tensor | None = None, cu_seqlens: Tensor | None = None, max_seqlen: int | None = None) -> Tensor:
        bsz, seqlen, dim = x.shape
        if q_w is not None:
            q = F.linear(x, q_w.to(x.dtype)).reshape(bsz, seqlen, self.num_heads, self.head_dim)
            k = F.linear(x, k_w.to(x.dtype)).reshape(bsz, seqlen, self.num_kv_heads, self.head_dim)
            v = F.silu(F.linear(x, v_w.to(x.dtype)))  # GLU-V: zero-param nonlinearity on values
        else:
            q = self.c_q(x).reshape(bsz, seqlen, self.num_heads, self.head_dim)
            k = self.c_k(x).reshape(bsz, seqlen, self.num_kv_heads, self.head_dim)
            v = F.silu(self.c_v(x))  # GLU-V: zero-param nonlinearity on values
        ...
        if cu_seqlens is not None:
            ...
            k_flat = k.reshape(bsz * seqlen, self.num_kv_heads, self.head_dim)
            v_flat = v.reshape(bsz * seqlen, self.num_kv_heads, self.head_dim)
            y_flat = flash_attn_3_varlen_func(
                q_flat, k_flat, v_flat, cu_seqlens, cu_seqlens,
                max_seqlen, max_seqlen, causal=True)
            y = y_flat.reshape(bsz, seqlen, self.num_heads, self.head_dim)
        else:
...
class SmearGate(nn.Module):
    ...
    def forward(self, x: Tensor, input_ids: Tensor | None = None) -> Tensor:
        ...
        blend = (1 - g) * x + g * x_prev
        if input_ids is not None:
            bos_mask = (input_ids == 1).unsqueeze(-1)  # [B, T, 1]
            blend = torch.where(bos_mask, x, blend)
        return blend
...
class Block(nn.Module):
    def __init__(
        ...
        if dtg:
            ...
            self.dtg_gate = None
    def forward(self, x: Tensor, x0: Tensor, q_w: Tensor | None = None, k_w: Tensor | None = None, v_w: Tensor | None = None, out_w: Tensor | None = None, up_w: Tensor | None = None, down_w: Tensor | None = None, v_embed: Tensor | None = None, cu_seqlens: Tensor | None = None, max_seqlen: int | None = None) -> Tensor:
        mix = self.resid_mix.to(dtype=x.dtype)
        x_in = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
        attn_out = self.attn(self.attn_norm(x_in) * self.ln_scale_factor, q_w, k_w, v_w, out_w, v_embed=v_embed, cu_seqlens=cu_seqlens, max_seqlen=max_seqlen)
...
class GPT(nn.Module):
    def __init__(
        ...
        ve_layers: str = "9,10",
        use_banks: bool = True,
        bos_reset_enabled: bool = False,
    ):
        super().__init__()
        self.use_banks = use_banks
        self.bos_reset_enabled = bos_reset_enabled
    ...
    def _run_blocks(self, x: Tensor, x0: Tensor, input_ids: Tensor, cu_seqlens: Tensor | None = None, max_seqlen: int | None = None) -> Tensor:
        n = self.num_layers
        skips: list[Tensor] = []
        ve_cache: dict = {}
        for i in range(self.num_encoder_layers):
            ...
            if self.use_banks:
                x = self.blocks[i](x, x0,
                    ...
                    v_embed=ve, cu_seqlens=cu_seqlens, max_seqlen=max_seqlen)
            else:
                x = self.blocks[i](x, x0, v_embed=ve, cu_seqlens=cu_seqlens, max_seqlen=max_seqlen)
            skips.append(x)
        for i in range(self.num_decoder_layers):
            ...
            if self.use_banks:
                ...
                x = self.blocks[bi](x, x0, v_embed=ve, cu_seqlens=cu_seqlens, max_seqlen=max_seqlen)
        return x
    def forward(self, input_ids: Tensor, target_ids: Tensor, cu_seqlens: Tensor | None = None, max_seqlen: int | None = None) -> Tensor:
        x = self.tok_emb(input_ids)
        if self.bigram is not None:
            x = x + self.bigram(input_ids)
        x = F.rms_norm(x, (x.size(-1),))
        x = self.smear(x, input_ids)
        x0 = x
        x = self._run_blocks(x, x0, input_ids, cu_seqlens=cu_seqlens, max_seqlen=max_seqlen)
        x = self.final_norm(x)
        ...
        return main_loss
    def forward_logits(self, input_ids: Tensor, cu_seqlens: Tensor | None = None, max_seqlen: int | None = None, slot_vector: Tensor | None = None, return_hidden: bool = False) -> Tensor:
        """Return logits (bsz, seq_len, vocab) without computing loss.
        slot_vector: optional [D] vector added to final hidden states (SLOT TTT).
        return_hidden: if True, return (logits, hidden) where hidden is pre-slot states."""
        x = self.tok_emb(input_ids)
        if self.bigram is not None:
            x = x + self.bigram(input_ids)
        x = F.rms_norm(x, (x.size(-1),))
        ...
        x = self._run_blocks(x, x0, input_ids, cu_seqlens=cu_seqlens, max_seqlen=max_seqlen)
        x = self.final_norm(x)
        hidden = x  # [B, T, D] — pre-slot hidden states for TTT gradient
        if slot_vector is not None:
            x = x + slot_vector.to(x.dtype)  # [D] broadcasts over [B, T, D]
        if self.tie_embeddings:
...
def eval_val_sliding(
    ...
    with torch.inference_mode():
        for bi in range(0, len(my_windows), batch_seqs):
            ...
            cu_seqlens, max_seqlen = (compute_cu_seqlens(x_batch) if args.bos_reset_enabled else (None, None))
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                logits = compiled_logits(x_batch, cu_seqlens, max_seqlen)
            nll = F.cross_entropy(
...
def eval_val_slot_ttt(
    ...
    """Score-first SLOT TTT eval: after scoring each batch, take AdamW steps on a single
    output-layer vector using the already-scored hidden states. Zero artifact cost —
    slot_vector is runtime-only (zero-initialized, not serialized)."""
    seq_len = eval_seq_len or args.train_seq_len
    local_batch_tokens = args.val_batch_size // (world_size * grad_accum_steps)
    if local_batch_tokens < seq_len:
        raise ValueError(
            f"VAL_BATCH_SIZE too small for SLOT TTT eval; got {args.val_batch_size}"
        )
    local_batch_seqs = local_batch_tokens // seq_len
    total_seqs = (val_tokens.numel() - 1) // seq_len
    seq_start = (total_seqs * rank) // world_size
    seq_end = (total_seqs * (rank + 1)) // world_size
    # SLOT vector: zero-init, float32, adapts across val set (never serialized)
    model_dim = base_model.tok_emb.embedding_dim
    slot_vector = torch.zeros(model_dim, device=device, dtype=torch.float32, requires_grad=True)
    slot_opt = torch.optim.AdamW([slot_vector], lr=args.slot_ttt_lr, betas=(0.9, 0.999), weight_decay=0.0)
    val_loss_sum = torch.zeros((), device=device, dtype=torch.float64)
    val_token_count = torch.zeros((), device=device, dtype=torch.float64)
    val_byte_count = torch.zeros((), device=device, dtype=torch.float64)
    base_model.eval()
    for batch_seq_start in range(seq_start, seq_end, local_batch_seqs):
        ...
        y = local[1:].reshape(-1, seq_len)
        cu_seqlens, max_seqlen = (compute_cu_seqlens(x) if args.bos_reset_enabled else (None, None))
        # Score-first: evaluate with current (detached) slot_vector, capture hidden states.
        # Using no_grad (not inference_mode) so hidden tensor is usable in subsequent autograd.
        with torch.no_grad():
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                logits, hidden = base_model.forward_logits(
                    x, cu_seqlens, max_seqlen,
                    slot_vector=slot_vector.detach().to(torch.bfloat16),
                    return_hidden=True,
                )
        ...
        token_bytes += (has_leading_space_lut[tgt_ids] & ~is_boundary_token_lut[prev_ids]).to(dtype=torch.int16)
        val_byte_count += token_bytes.to(torch.float64).sum()
        # TTT update: gradient flows only through slot_vector (model weights frozen).
        # hidden is [B, T, D] fp32 — no grad through model body.
        h = hidden.float().detach()
        for _ in range(args.slot_ttt_steps):
            slot_opt.zero_grad()
            h_slot = h + slot_vector  # [D] broadcasts to [B, T, D]; grad tracked via slot_vector
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                if base_model.tie_embeddings:
                    logits_ttt = F.linear(h_slot.to(torch.bfloat16), base_model.tok_emb.weight.detach())
                else:
                    logits_ttt = F.linear(h_slot.to(torch.bfloat16), base_model.lm_head.weight.detach())
                logits_ttt = base_model.logit_softcap * torch.tanh(logits_ttt / base_model.logit_softcap)
            ttt_loss = F.cross_entropy(logits_ttt.reshape(-1, args.vocab_size).float(), y.reshape(-1), reduction="mean")
            ttt_loss.backward()
            slot_opt.step()
    if dist.is_available() and dist.is_initialized():
        dist.all_reduce(val_loss_sum, op=dist.ReduceOp.SUM)
...
def main() -> None:
    ...
    _eval_only = bool(args.eval_from_checkpoint)
    if _eval_only:
        _ckpt = torch.load(args.eval_from_checkpoint, map_location="cpu", weights_only=False)
        quant_blob = _ckpt["quant_blob"]
        unbanked_sd = _ckpt["unbanked_sd"]
        code_bytes = os.path.getsize(__file__)
        log0(f"eval_from_checkpoint: {args.eval_from_checkpoint}")
        if master_process:
        ...
        base_model = GPT(
            ...
            bos_reset_enabled=args.bos_reset_enabled,
        ).to(device).bfloat16()
        ...
        while True:
            ...
            for micro_step in range(grad_accum_steps):
                ...
                cu_seqlens, max_seqlen = (compute_cu_seqlens(x) if args.bos_reset_enabled else (None, None))
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                    loss = model(x, y, cu_seqlens, max_seqlen)
    ...
    if args.checkpoint_dir and master_process:
        os.makedirs(args.checkpoint_dir, exist_ok=True)
        ckpt_path = os.path.join(args.checkpoint_dir, f"{args.run_id}.ckpt")
        torch.save({
            "quant_blob": quant_blob,
            "unbanked_sd": {k: v.cpu() for k, v in unbanked_sd.items()},
        }, ckpt_path)
    ...
    if args.slot_ttt_enabled:
        torch.cuda.synchronize()
        t_slot = time.perf_counter()
        slot_val_loss, slot_val_bpb = eval_val_slot_ttt(
        ...
        torch.cuda.synchronize()
        log0(
            f"final_slot_ttt val_loss:{slot_val_loss:.4f} val_bpb:{slot_val_bpb:.4f} "
            f"lr:{args.slot_ttt_lr} steps:{args.slot_ttt_steps} "
            f"eval_time:{1000.0 * (time.perf_counter() - t_slot):.0f}ms"
        )
        log0(f"final_slot_ttt_exact val_loss:{slot_val_loss:.8f} val_bpb:{slot_val_bpb:.8f}")
