...
class Hyperparameters:
    ...
    linear_attn_enabled = bool(int(os.environ.get("LINEAR_ATTN_ENABLED", "0")))
    linear_attn_features = int(os.environ.get("LINEAR_ATTN_FEATURES", "64"))
...
class CausalSelfAttention(nn.Module):
    ...
    def _performer_phi(self, x: Tensor) -> Tensor:
        """ReLU random feature map: maps [..., head_dim] → [..., r], all positive."""
        omega = self._performer_omega.to(dtype=x.dtype)
        return F.relu(x @ omega) + 1e-6
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, cu_seqlens: Tensor | None = None, max_seqlen: int | None = None) -> Tensor:
        ...
        return main_loss
    def forward_logits(self, input_ids: Tensor, cu_seqlens: Tensor | None = None, max_seqlen: int | None = None) -> Tensor:
